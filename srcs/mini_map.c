...
/*   Updated: 2023/05/31 15:42:22 by aharrass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_mini_map(t_mlx *mlx)
{
	...
	mlx->mini_map.addr = mlx_get_data_addr(mlx->mini_map.img,
			&mlx->mini_map.bpp,
			&mlx->mini_map.line_length,
			&mlx->mini_map.endian);
	render_map(mlx);
	render_player(mlx);
	crop_map(mlx);
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->win, mlx->mini_map.img, 20, 20);
...
void	fill_map(t_mlx *mlx, int *x, int *y)
{
	int	tilex;
	int	tiley;
